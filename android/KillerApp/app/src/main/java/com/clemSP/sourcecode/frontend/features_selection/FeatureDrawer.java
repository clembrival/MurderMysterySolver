...
public class FeatureDrawer
{
    ...
    private void onFeaturesDrawerOpened(Activity activity)
    {
        ...
        int indexToShow = predictWeapon ? AppAttribute.Murderer.getIndex()
                : AppAttribute.Weapon.getIndex();
        ...
        if(mFeaturesAdapter.getCount() < AppAttribute.getNumAttributes() - 1)
        {
            mFeaturesAdapter.insert(activity.getResources()
                            .getStringArray(R.array.features_array)[indexToShow],
                    predictWeapon ? indexToShow-1 : indexToShow);
        }
    ...
    /**
      * @return a boolean array which elements contain true iff
