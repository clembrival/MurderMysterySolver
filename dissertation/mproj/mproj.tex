...
While KStar looks for the closest training example using the ``complexity of transforming one instance into another'' as a distance measure, \cite{kstar}, IBk uses by default the Euclidean distance. \cite[Chapter~11]{wekabook}\par

Since both classifiers seemed to have similar performances, IBk was arbitrarily chosen. As mentioned in section \ref{data_process}, a new instance of the \verb|IBk| class was created, trained on the initial dataset, and output to a .model file. This file was then stored in the Android project as an asset, so the classifier object could be retreived within the app.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Story Details Input}

The user experiment described in section \ref{wireframes} showed two main groups of users: those who favoured a clear, simple interface, which did not require much effort to read or understand, and those who enjoyed an interactive interface with images better. Because it did not seem possible to create one interface which could satisfy all the users, several layouts were implemented, so users could customize their own application by changing the settings.
